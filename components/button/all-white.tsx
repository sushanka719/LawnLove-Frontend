...
const allWhiteButtonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-xl border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "border-[1.2px] border-[#206944E5] bg-[#FFFCF5] text-[#195134] shadow-sm hover:bg-[#FFFCF5] opacity-40",
      },
      size: {
...
function AllWhiteButton({
  className,
  variant = "default",
  size = "default",
  ...props
}: ButtonPrimitive.Props & VariantProps<typeof allWhiteButtonVariants>) {
  return (
    <ButtonPrimitive
      data-slot="button-all-white"
      className={cn(allWhiteButtonVariants({ variant, size, className }))}
      {...props}
    />
  );
}

export { AllWhiteButton, allWhiteButtonVariants };
