...
const bigGradientDarkButtonVariants = cva(
...
function BigGradientDarkButton({
  className,
  variant = "default",
  size = "default",
  ...props
}: ButtonPrimitive.Props & VariantProps<typeof bigGradientDarkButtonVariants>) {
  return (
    <ButtonPrimitive
      data-slot="button-gradient-dark"
      className={cn(bigGradientDarkButtonVariants({ variant, size, className }))}
      {...props}
    />
  );
}

export { BigGradientDarkButton, bigGradientDarkButtonVariants };
